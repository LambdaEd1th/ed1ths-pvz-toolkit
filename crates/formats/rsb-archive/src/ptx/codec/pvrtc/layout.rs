pub(super) const MORTON_TABLE: [i32; 256] = [
...
pub(super) const BILINEAR_FACTORS: [[u8; 4]; 16] = [
    [4, 4, 4, 4],
    [2, 6, 2, 6],
    [8, 0, 8, 0],
    [6, 2, 6, 2],
...
pub(super) const WEIGHTS: [u8; 32] = [
    8, 0, 8, 0, 5, 3, 5, 3, 3, 5, 3, 5, 0, 8, 0, 8, 8, 0, 8, 0, 4, 4, 4, 4, 4, 4, 0, 0, 0, 8, 0, 8,
];

pub(super) fn get_morton_number(x: i32, y: i32) -> usize {
...
pub(super) fn get_morton_number_rect(x: i32, y: i32, blocks_x: i32, blocks_y: i32) -> usize {
    let minimum = blocks_x.min(blocks_y);
    let low_mask = minimum - 1;
    let low = get_morton_number(x & low_mask, y & low_mask);
    let low_bits = (minimum as u32).trailing_zeros();
    let high = if blocks_x > blocks_y {
        x >> low_bits
    } else {
        y >> low_bits
    };
    low | ((high as usize) << (2 * low_bits))
}
