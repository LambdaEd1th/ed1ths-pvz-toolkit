...
use rsb_codec::{PtxDecoder, PtxEncoder, PtxFormat, decode_pvrtc_4bpp, encode_pvrtc_4bpp};

#[test]
fn encodes_nonzero_pvrtc_packets_and_round_trips_constant_color() {
    let expected = [72, 136, 200, 255];
    let image = DynamicImage::ImageRgba8(ImageBuffer::from_pixel(8, 8, Rgba(expected)));
    let encoded = encode_pvrtc_4bpp(&image, true).unwrap();

    assert_eq!(encoded.len(), 32);
    assert!(encoded.iter().any(|&byte| byte != 0));

    let decoded = decode_pvrtc_4bpp(&encoded, 8, 8).unwrap().to_rgba8();
...
fn round_trips_rgba_gradient_with_bounded_error() {
    ...
    let encoded = encode_pvrtc_4bpp(&DynamicImage::ImageRgba8(image.clone()), true).unwrap();
    let decoded = decode_pvrtc_4bpp(&encoded, 16, 16).unwrap().to_rgba8();
...
fn ptx_dispatch_encodes_pvrtc_and_separate_alpha() {
    ...
    let rgba = PtxEncoder::encode(&image, PtxFormat::Pvrtc4BppRgba, false).unwrap();
    assert_eq!(rgba.len(), 32);
    let rgba_a8 = PtxEncoder::encode(&image, PtxFormat::Pvrtc4BppRgbaA8, false).unwrap();
    assert_eq!(rgba_a8.len(), 32 + 64);

    let decoded = PtxDecoder::decode(&rgba_a8, 8, 8, 148, None, Some(64), None, false)
        .unwrap()
        .to_rgba8();
    for (expected, actual) in image.to_rgba8().pixels().zip(decoded.pixels()) {
...
fn rejects_dimensions_outside_the_supported_pvrtc_layout() {
    let non_power_of_two = DynamicImage::new_rgba8(12, 12);
    assert!(encode_pvrtc_4bpp(&non_power_of_two, true).is_err());
}

#[test]
fn supports_rectangular_power_of_two_textures() {
    let image = DynamicImage::ImageRgba8(ImageBuffer::from_fn(8, 16, |x, y| {
        Rgba([(x * 31) as u8, (y * 15) as u8, ((x + y) * 11) as u8, 255])
    }));
    let encoded = encode_pvrtc_4bpp(&image, true).unwrap();
    assert_eq!(encoded.len(), 64);
    let decoded = decode_pvrtc_4bpp(&encoded, 8, 16).unwrap();
...
fn matches_twinning_pvrtc_reference_packets() {
    ...
    assert_eq!(encode_pvrtc_4bpp(&image, true).unwrap(), expected);
}
