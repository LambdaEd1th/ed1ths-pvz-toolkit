...
fn decode_color_values(data: u128, n_values: u32, n_bits_for_color_data: u32) -> [u8; 18] {
    ...
    match encoding.encoding {
        ...
        IntegerEncodingType::Trit => {
            let mut written = 0;
            while written < out_range.len() {
                for result in decode_trit_block(&mut color_stream, encoding.num_bits) {
                    if written == out_range.len() {
                        break;
                    }
                    out_range[written] = result.decode_color(encoding.num_bits);
                    written += 1;
                }
            }
        }
        IntegerEncodingType::Quint => {
            let mut written = 0;
            while written < out_range.len() {
                for result in decode_quint_block(&mut color_stream, encoding.num_bits) {
                    if written == out_range.len() {
                        break;
                    }
                    out_range[written] = result.decode_color(encoding.num_bits);
...
fn unquantize_texel_weights(
    ...
    match encoding.encoding {
        ...
        IntegerEncodingType::Trit => {
            let mut written = 0;
            while written < unquantized_range.len() {
                for result in decode_trit_block(weights_stream, encoding.num_bits) {
                    if written == unquantized_range.len() {
                        break;
                    }
                    unquantized_range[written] = result.decode_weight(encoding.num_bits);
                    written += 1;
                }
            }
        }
        IntegerEncodingType::Quint => {
            let mut written = 0;
            while written < unquantized_range.len() {
                for result in decode_quint_block(weights_stream, encoding.num_bits) {
                    if written == unquantized_range.len() {
                        break;
                    }
                    unquantized_range[written] = result.decode_weight(encoding.num_bits);
