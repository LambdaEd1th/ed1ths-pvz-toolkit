...
/// Encodes an image as raw ASTC LDR blocks without C/C++ or FFI dependencies.
///
...
pub fn encode_astc(
    ...
    for block_y in 0..rgba.height().div_ceil(block_height) {
        for block_x in 0..rgba.width().div_ceil(block_width) {
            let pixels = read_block(&rgba, block_x, block_y, block_width, block_height);
...
fn read_block(
    ...
    for y in 0..block_height {
        for x in 0..block_width {
            let source_x = (block_x * block_width + x).min(image.width().saturating_sub(1));
            let source_y = (block_y * block_height + y).min(image.height().saturating_sub(1));
