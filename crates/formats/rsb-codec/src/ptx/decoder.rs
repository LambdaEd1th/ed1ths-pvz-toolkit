...
use crate::ptx::codec::astc::decode_astc;
...
impl PtxDecoder {
    pub fn decode_payload(payload: &PtxPayload<'_>) -> Result<RgbaImage> {
        ...
        match descriptor.format {
            ...
            PtxFormat::Pvrtc4BppRgba => {
                decode_pvrtc_4bpp(payload.color(), descriptor.width, descriptor.height)
                    .map(DynamicImage::into_rgba8)
                    .map_err(PtxError::from)
            }
            PtxFormat::Pvrtc4BppRgbaA8 => decode_pvrtc_4bpp_a8(
                payload.color(),
                payload
                    .alpha()
    ...
    /// Compatibility wrapper for the original manifest-field decoder.
    ...
    pub fn decode(
        ...
        let metadata = PtxRsbMetadata {
            ...
            channel_order: if is_powervr {
                ChannelOrder::Bgra
            } else {
                ChannelOrder::Rgba
            },
        };
        let descriptor = PtxDescriptor::from_rsb(width, height, metadata).map_err(to_rsb_error)?;
        Self::decode_with_descriptor(data, descriptor)
