...
//! by plain `serde_json`.
//!
//! # Optional encrypted RTON support
//!
//! Enable the `crypto` Cargo feature to expose the `crypto` module,
//! `encrypt_data`, and `decrypt_data`. The feature is disabled by default so
//! core RTON reading and writing does not pull in cryptography dependencies.
