...
- Optionally provide Rijndael-192-CBC helpers for encrypted PvZ2 RTON payloads.
...
serde_rton = { git = "https://github.com/LambdaEd1th/ed1ths-pvz-toolkit" }
```

### Cargo features

`serde_rton` has no default features. Core standard/compact RTON reading and
writing therefore does not pull in cryptography dependencies.

| Feature | Default | Adds |
| --- | --- | --- |
| `crypto` | No | PvZ2 encrypted RTON helpers and their Rijndael, MD5, and hex dependencies |

Enable encrypted RTON support explicitly:
...
the plaintext to `from_bytes`. Enable the `crypto` Cargo feature to make this
module and the root-level `encrypt_data` / `decrypt_data` exports available.
...
- `src/crypto.rs`: encrypted RTON helpers, available with the `crypto` feature.
