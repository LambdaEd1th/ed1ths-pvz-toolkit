use wem_audio_worker::{ConvertRequest, ConvertedAudio, PrepareRequest, PreparedAudio};
...
pub async fn prepare_audio(request: PrepareRequest) -> Result<PreparedAudio, String> {
    #[cfg(not(target_arch = "wasm32"))]
    {
        let (sender, receiver) = futures_channel::oneshot::channel();
        audio_pool().spawn(move || {
            let result = wem_audio_worker::prepare(request).map_err(|error| error.to_string());
            let _ = sender.send(result);
        });
        receiver
            .await
            .map_err(|_| "音频后台任务意外终止".to_string())?
    }
...
pub async fn convert_audio(request: ConvertRequest) -> Result<ConvertedAudio, String> {
    #[cfg(not(target_arch = "wasm32"))]
    {
        let (sender, receiver) = futures_channel::oneshot::channel();
        audio_pool().spawn(move || {
            let result = wem_audio_worker::convert(request).map_err(|error| error.to_string());
            let _ = sender.send(result);
        });
        receiver
            .await
    ...
    #[cfg(target_arch = "wasm32")]
    {
        request_worker("convert", request).await
    }
}

#[cfg(not(target_arch = "wasm32"))]
fn audio_pool() -> &'static rayon::ThreadPool {
    static POOL: OnceLock<rayon::ThreadPool> = OnceLock::new();
    POOL.get_or_init(|| {
        rayon::ThreadPoolBuilder::new()
            .num_threads(2)
            .thread_name(|index| format!("wem-audio-{index}"))
            .build()
            .expect("failed to create WEM audio thread pool")
    })
}
    ...
    O: DeserializeOwned,
{
    ...
    let promise = WORKER.with(|slot| {
        ...
        slot.as_mut()
            .expect("WEM worker was initialized")
...
async fn decode_web_response<T: DeserializeOwned>(promise: Promise) -> Result<T, String> {
    ...
    if !ok {
        return Err(Reflect::get(&envelope, &JsValue::from_str("error"))
            ...
            .unwrap_or_else(|| "WEM Audio Worker 执行失败".to_string()));
    }
    let response =
...
fn worker_url() -> String {
    let root = crate::WEM_ASSETS.to_string();
    format!(
        "{}/worker/wem-worker.js?v=20260729-wem-audio-1",
...
fn worker_error_message(event: &JsValue) -> String {
    Reflect::get(event, &JsValue::from_str("message"))
        ...
        .unwrap_or_else(|| "WEM Audio Worker 加载失败".to_string())
}
