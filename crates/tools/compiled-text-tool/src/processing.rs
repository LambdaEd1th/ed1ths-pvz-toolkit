use compiled_text_worker::{DecodeRequest, DecodeResponse, EncodeRequest, EncodeResponse};
...
pub async fn decode_document(request: DecodeRequest) -> Result<DecodeResponse, String> {
    #[cfg(not(target_arch = "wasm32"))]
    {
        let (sender, receiver) = futures_channel::oneshot::channel();
        worker_pool().spawn(move || {
            let result = compiled_text_worker::decode(request).map_err(|error| error.to_string());
            let _ = sender.send(result);
        });
        receiver
            .await
            .map_err(|_| "Compiled Text 后台任务意外终止".to_string())?
    }

    #[cfg(target_arch = "wasm32")]
    {
        request_worker("decode", request).await
    }
}

pub async fn encode_document(request: EncodeRequest) -> Result<EncodeResponse, String> {
    #[cfg(not(target_arch = "wasm32"))]
    {
        let (sender, receiver) = futures_channel::oneshot::channel();
        worker_pool().spawn(move || {
            let result = compiled_text_worker::encode(request).map_err(|error| error.to_string());
            let _ = sender.send(result);
        });
        receiver
            .await
    ...
    #[cfg(target_arch = "wasm32")]
    {
        request_worker("encode", request).await
    }
}
...
fn worker_pool() -> &'static rayon::ThreadPool {
    ...
    POOL.get_or_init(|| {
        rayon::ThreadPoolBuilder::new()
            .num_threads(2)
            .thread_name(|index| format!("compiled-text-{index}"))
            .build()
            .expect("failed to create Compiled Text worker pool")
    })
}
    ...
    O: DeserializeOwned,
{
    ...
    let promise = WORKER.with(|slot| {
        ...
        slot.as_mut()
            .expect("Compiled Text worker was initialized")
...
async fn decode_web_response<T: DeserializeOwned>(promise: Promise) -> Result<T, String> {
    ...
    if !ok {
        return Err(Reflect::get(&envelope, &JsValue::from_str("error"))
            ...
            .unwrap_or_else(|| "Compiled Text Worker 执行失败".to_string()));
    }
    let response =
...
fn worker_url() -> String {
    let root = crate::COMPILED_TEXT_ASSETS.to_string();
    format!(
        "{}/worker/compiled-text-worker.js?v=20260824-compiled-text-1",
...
fn worker_error_message(event: &JsValue) -> String {
    Reflect::get(event, &JsValue::from_str("message"))
        ...
        .unwrap_or_else(|| "Compiled Text Worker 加载失败".to_string())
}
